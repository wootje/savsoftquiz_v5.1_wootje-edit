...
if ($unicodemap) {
  ...
  File size: <?php
		echo round(filesize($fontfile) / 1024, 3); ?>KB &mdash;
  Memory: <?php
		echo(memory_get_peak_usage(true) / 1024); ?>KB &mdash;
  Time: <?php
		echo round(microtime(true) - $t, 4); ?>s
  <br/>
  <a href="make_subset.php?fontfile=<?php
  echo $fontfile; ?>&amp;name=<?php
  echo urlencode($font->getFontSubfamilyID()); ?>">Make a subset of this font</a>
</span><h1><?php
		echo $font->getFontFullName(); ?></h1><h3><?php
		echo $font->getFontVersion(); ?></h3>
	<hr/>
	<div id="tabs">
		<ul>
			...
			foreach ($font->getTable() as $entry) {
				...
				<li>
					<a <?php
					if (!$data) { ?> style="color: #ccc;" <?php
					} ?> href="#tabs-<?php
					echo preg_replace("/[^a-z0-9]/i", "_", $tag); ?>"><?php
						echo $tag; ?></a>
				</li>
			<?php
			} ?>
		...
		<div id="tabs-header">
			<pre><?php
				echo var_export($font->header->data, true); ?></pre>
		</div>
		<?php
		foreach ($font->getTable() as $table) {
			...
			?>
			<div id="tabs-<?php
			echo preg_replace("/[^a-z0-9]/i", "_", $tag); ?>">
				<?php
				if ($data) {
