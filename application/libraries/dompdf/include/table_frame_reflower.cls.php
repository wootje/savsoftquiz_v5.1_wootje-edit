	...
	protected function _assign_widths()
	{
		...
		if ($width !== "auto") {
			...
			if ($preferred_width > $min_width)
				$width = $preferred_width; else
				$width = $min_width;
		} else {
			if ($max_width + $delta < $cb["w"])
				$width = $max_width; else if ($cb["w"] - $delta > $min_width)
				$width = $cb["w"] - $delta; else
				$width = $min_width;
			...
			if ($absolute_used > 0 && $percent_used == 0) {
				...
				foreach (array_keys($columns) as $i) {
					if ($columns[$i]["absolute"] > 0 && count($auto))
						$cellmap->set_column_width($i, $columns[$i]["min-width"]); else if (count($auto))
						$cellmap->set_column_width($i, $columns[$i]["min-width"] + $increment); else {
