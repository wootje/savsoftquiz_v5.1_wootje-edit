...
class PDFLib_Adapter implements Canvas
{
	...
	protected function _set_line_style($width, $cap, $join, $dash)
	{
		if (count($dash) == 1)
			$dash[] = $dash[0];
		if (count($dash) > 1)
			$this->_pdf->setdashpattern("dasharray={" . implode(" ", $dash) . "}"); else
	 ...
	 */
	function page_text($x, $y, $text, $font, $size, $color = array(
		0,
		0,
		0
	...
	function stream($filename, $options = null)
	{
		...
		if (isset($options["compress"]) && $options["compress"] != 1)
			$this->_pdf->set_value("compress", 0); else
	...
	function output($options = null)
	{
		...
		$this->_close();
		if (self::$IN_MEMORY)
			$data = $this->_pdf->get_buffer(); else {
