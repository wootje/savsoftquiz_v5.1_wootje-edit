...
<div class="container">
	...
	<div class="row">
		<div class="col-sm-3"><!--left col-->
			...
			<h3><?php
				echo $this->lang->line('members'); ?> (<?php
				echo $group['no_member']; ?>)</h3>
			<div style="height:300px;width:100%;overflow-y:auto;background:#fcfcfc;border:1px solid #ddd;padding:5px;">
				...
				foreach ($members as $k => $val) {
					...
					<div style="border-bottom:1px solid #dddddd;padding:2px;">
						...
						?>
						<p><?php
							echo $val['email']; ?><br> <span style="color:#666666;font-size:11px;"><?php
								echo $this->lang->line('joined'); ?><?php
								echo $val['joined_date']; ?></span>
						</p>
					</div>
					<?php
				}
		...
		<div class="col-sm-9">
			<h3><?php
				echo $this->lang->line('news_feed'); ?></h3>
			<?php
			if (count($feed) == 0) {
			...
			foreach ($feed as $k => $fd) {
				...
				<div class="media" style="border-bottom:1px solid #dddddd;padding-bottom:4px;">
					<div class="media-body">
						<p class="notification-desc"><?php
							echo $fd['feed']; ?> </p>
						<div class="notification-meta">
							<small class="timestamp"><?php
								echo $fd['feed_date']; ?></small>
						</div>
					</div>
				</div>
				<?php
			...
			if ($limit != 0) { ?>
				<a href="<?php
				echo site_url('social_group/view/' . $sg_id . '/0/' . $back); ?>" class="btn btn-primary"><?php
					echo $this->lang->line('back'); ?></a>&nbsp;&nbsp;
				<?php
			}
			$next = $limit + ($this->config->item('number_of_rows')); ?>
			<a href="<?php
			echo site_url('social_group/view/' . $sg_id . '/0/' . $next); ?>" class="btn btn-primary"><?php
				echo $this->lang->line('next'); ?></a>
		</div>
	</div>
</div><!--/col-9--></div><!--/row--><!-- modal1 --><!-- Modal -->
		...
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title"> <?php
					echo $this->lang->line('invite'); ?></h4>
			</div>
			<div class="modal-body">
				<p>
				<form method="post" action="<?php
				echo site_url('social_group/invite/' . $sg_id); ?>">
					<div class="form-group">
						<input type="text" name="email" value="" placeholder="<?php
						echo $this->lang->line('email_comma'); ?>" class="form-control">
					</div>
					<div class="form-group">
						<input type="text" name="custom_message" value="" placeholder="<?php
						echo $this->lang->line('custom_message'); ?>" class="form-control">
					</div>
					<div class="form-group">
	...
	<div class="modal-dialog">
		...
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal">&times;</button>
				<h4 class="modal-title"> <?php
					echo $this->lang->line('add_user'); ?></h4>
			</div>
			<div class="modal-body">
				<p>
				<form method="post" action="<?php
				echo site_url('social_group/add_user/' . $sg_id); ?>">
					<div class="form-group">
						<input type="text" name="email" value="" placeholder="<?php
						echo $this->lang->line('email_comma'); ?>" class="form-control">
					</div>
