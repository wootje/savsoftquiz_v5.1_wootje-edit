...
<head>
	...
	<!-- bootstrap css -->
	<link href="<?php
	echo base_url('bootstrap/css/bootstrap.min.css'); ?>" rel="stylesheet">
	...
	<!-- jquery -->
	<script src="<?php
	echo base_url('js/jquery.js'); ?>"></script>
	<!-- custom javascript -->
	<script src="<?php
	echo base_url('js/basic.js'); ?>"></script>
	<!-- bootstrap js -->
	<script src="<?php
	echo base_url('bootstrap/js/bootstrap.min.js'); ?>"></script>
	<link rel="stylesheet" href="<?php
	echo base_url('css/jquery.mobile-1.4.5.min.css'); ?>">
	<script src="<?php
	echo base_url('js/jquery.mobile-1.4.5.min.js'); ?>"></script>
</head>
<body>
<style>
	td {
...
<div class="container">
	...
	<div class="row" style="margin-top:5px;">
		<div class="col-md-8">
			...
			echo site_url('quiz/submit_quiz/' . $quiz['rid']); ?>" id="quiz_form">
				<input type="hidden" name="rid" value="<?php
				echo $quiz['rid']; ?>"> <input type="hidden" name="noq" value="<?php
				echo $quiz['noq']; ?>"> <input type="hidden" name="individual_time" id="individual_time" value="<?php
				echo $quiz['individual_time']; ?>">
				<?php
				$abc = array(
					'0' => 'A',
					'1' => 'B',
							...
							if ($question['question_type'] == $this->lang->line('multiple_choice_single_answer')) {
								...
								foreach ($options as $ok => $option) {
									if ($option['qid'] == $question['qid']) {
										?>
										<div class="op op_style" onClick="checkrad('answer_value<?php
										echo $qk . '-' . $i; ?>');">
											<table>
												<tr>
													<td>
								...
								foreach ($options as $ok => $option) {
									if ($option['qid'] == $question['qid']) {
										...
										<div class="op op_style">
											<div class="checkbox_layer" onClick="checkrad2('answer_value<?php
											echo $qk . '-' . $i; ?>');"></div>
											<table>
												<tr>
													<td>
	...
	<script type="text/javascript">window.onload = CreateTimer("timer", <?php echo $seconds;?>);</script>
				...
				<b> <?php
					echo $this->lang->line('questions'); ?></b>
				<div>
					<?php
					for ($j = 0; $j < $quiz['noq']; $j++) {
						?>
						<div class="qbtn" onClick="javascript:show_question('<?php
						echo $j; ?>');" id="qbtn<?php
						echo $j; ?>"><?php
							echo($j + 1); ?></div>
						<?php
...
<div class="footer_buttons">
	<button class="btn btn-warning" onClick="javascript:review_later();" style="margin-top:2px;"><?php
		echo $this->lang->line('rl'); ?></button>
	<button class="btn btn-info" onClick="javascript:clear_response();" style="margin-top:2px;"><?php
		echo $this->lang->line('clear'); ?></button>
	<button class="btn btn-success" id="backbtn" style="visibility:hidden;" onClick="javascript:show_back_question2();" style="margin-top:2px;"><?php
		echo $this->lang->line('back'); ?></button>
	<button class="btn btn-success" id="nextbtn" onClick="javascript:show_next_question2();" style="margin-top:2px;"><?php
		echo $this->lang->line('save_next'); ?></button>
	<button class="btn btn-danger" onClick="javascript:cancelmove();" style="margin-top:2px;"><?php
		echo $this->lang->line('submit_quiz'); ?></button>
</div>
<div style="position:fixed;top:40%;width:80px;cursor:pointer; color:#8A6D3B;height:30px;right:0px;z-index:1200;background:#FCF8E3;padding:5px;border:1px solid #FAEBCC;vertical-align:center" id="slideri">
	<span class="glyphicons glyphicons-expand"></span> <?php
	echo $this->lang->line('pallet'); ?></div>
<script>
	var ctime = 0;
	var ind_time = [];
	<?php
...
<div id="warning_div" style="padding:10px; position:fixed;z-index:1200;display:none;width:98%;border-radius:5px;height:120px; border:1px solid #dddddd;left:4px;top:70px;background:#ffffff;">
	<center><b> <?php
			echo $this->lang->line('really_Want_to_submit'); ?></b> <br><br> <span id="processing"></span>
		<a href="javascript:hidespiner();cancelmove();" class="btn btn-danger" style="cursor:pointer;"><?php
			echo $this->lang->line('cancel'); ?></a> &nbsp; &nbsp; &nbsp; &nbsp;
		<a href="javascript:submit_quiz2();" class="btn btn-info" style="cursor:pointer;"><?php
			echo $this->lang->line('submit_quiz'); ?></a>
	</center>
</div>
<div class="" id="loading_spin0" style="display:none;position:fixed;width:100%;height:100%;z-index:1000;background:#212121;color:#ffffff;opacity:0.8;text-align:center;top:0px;right:0px;"></div>
</body>
</html>
