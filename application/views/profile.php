	...
	<div class="row">
		<div class="col-sm-3"><!--left col-->
			...
			<div class="card shadow  py-2">
				<div class="card-heading" style="padding:5px;"><?php
					echo $this->lang->line('contact'); ?></div>
				<div class="card-body"><i class="fa fa-envelope fa-1x"></i> <?php
					echo $result['email']; ?> </div>
				<div class="card-body"><i class="fa fa-phone fa-1x"></i> <?php
					echo $result['contact_no']; ?> </div>
			</div>
			<ul class="list-group">
				<li class="list-group-item text-muted"><?php
		...
		<div class="col-sm-9">
			<h3> <?php
				echo $this->lang->line('category_prof'); ?> </h3>
			<div class="table-responsive">
				<table class="table table-striped">
					...
					<tr>
						...
						<th>
							<?php
							echo $this->lang->line('overall_percentage'); ?>
						</th>
						<th>
							<?php
							echo $this->lang->line('percentage_last_quiz'); ?>
						</th>
					</tr>
					<?php
					arsort($categories);
					foreach ($categories as $k => $val) {
						?>
						<tr <?php
						if (intval($val) <= 50){ ?>style="background-color:#f2dede;" <?php
						}else{ ?>style="background-color:#dff0d8;" <?php
						} ?> >
							<td>
								<?php
								echo $k; ?>
							...
							<td>
								<?php
								if (intval($category_recent[$k]) >= intval($val)) {
									?>
									<i class="fa fa-arrow-up" style="color:green;" title="<?php
									echo $this->lang->line('improving'); ?>"></i>
									<?php
								} else {
									?>
									<i class="fa fa-arrow-down" style="color:red;" title="<?php
									echo $this->lang->line('improving2'); ?>"></i>
									<?php
								}
								?>
								<?php
								if ($category_recent[$k] >= 100) {
									echo '100';
								} else {
			...
			<h3><?php
				echo $this->lang->line('questions_incorect'); ?></h3>
			<div class="table-responsive">
				<table class="table table-striped">
					<tbody>
					<tr>
					...
					foreach ($questions as $k => $qv) {
						...
						<tr>
							...
							<td>
								<?php
								echo substr(strip_tags($qv['question']), 0, 100); ?>
							</td>
							<td>
								<a href="#" data-toggle="modal" data-target="#myModal<?php
								echo $qv['qid']; ?>"><i class="fa fa-eye" title="View Question"></i></a>
							</td>
						</tr>
						<?php
					}
...
foreach ($questions as $k => $qv) {
	...
	<!-- Modal -->
	<div id="myModal<?php
	echo $qv['qid']; ?>" class="modal fade" role="dialog">
		<div class="modal-dialog">
			...
			<div class="modal-content">
				...
				<div class="modal-footer">
					<p><?php
						echo $qv['question_type']; ?></p>
					<p><?php
						echo $this->lang->line('percent_corrected'); ?>:
						<?php
						if ($qv['no_time_served'] != '0') {
