		...
		<div class="col-md-10">
			...
			<div class="container">
				<div style="margin-top:50px;margin-bottom:50px;">
					<h3><?php
						echo $this->lang->line('select_payment_gateway'); ?> </h3>
				</div>
			</div>
			<div id="exTab1" class="container">
				<ul class="nav nav-pills">
					...
					if ($this->config->item('paypal') == 'true') {
						?>
						<li class="">
							<a href="#1a" data-toggle="tab" class="active tabtitle slink" onClick="showtab('#1a',this);"><?php
								echo $this->lang->line('paypal'); ?></a>
						</li>
						<?php
					}
					...
					<div class="tab-pane active" id="1a">
						...
						if ($this->config->item('paypal') == 'true') {
							...
							echo $this->config->item('paypal_environment'); ?>paypal.com/cgi-bin/webscr" method="post">
								...
								echo $this->config->item('paypal_receiver'); ?>">
								<input type="hidden" name="currency_code" value="USD">
								<input type="hidden" name="item_name" value="<?php
								echo $this->lang->line('quiz_subscription'); ?>">
								<input type="hidden" name="notify_url" value="<?php
								echo site_url('payment_gateway_2/paypal_ipn'); ?>">
								<input type="hidden" name="custom" value="<?php
								echo $user['uid'] . '-' . str_replace(',', ':', $gids) . '-' . $quid; ?>">
								<input type="hidden" name="return" value="<?php
								echo site_url('payment_gateway_2/success'); ?>">
								<input type="hidden" name="cancel_return" value="<?php
								echo site_url('payment_gateway_2/cancel'); ?>">
								<input type="hidden" name="amount" value="<?php
								echo $pricek * $this->config->item('paypal_conversion'); ?>">
								<input class="btn btn-default" value="<?php
								echo $this->lang->line('paynow'); ?>" type="submit" name="submit" alt="">
							</form>
							<?php
						}
						?>
					...
					<div class="tab-pane" id="2a">
						...
						if ($this->config->item('checkout') == 'true') {
							...
							echo $this->config->item('checkout_environment'); ?>2checkout.com/checkout/purchase" method='post'>
								...
								echo $this->config->item('checkout_sid'); ?>"/>
								<input type='hidden' name='x_receipt_link_url' value="<?php
								echo site_url('payment_gateway_2/checkout_ipn/'); ?>"/>
								...
								echo $pricek * $this->config->item('checkout_conversion'); ?>"/>
								<input name='submit' type='submit' class="btn btn-default" value="<?php
								echo $this->lang->line('paynow'); ?>"/>
							</form>
							<?php
						}
						?>
					...
					<div class="tab-pane" id="3a">
						...
						if ($this->config->item('payumoney') == 'true') {
							...
							<form method="POST" action="https://secure.payu.in/_payment">
								...
								echo $MERCHANT_KEY; ?>"/> <input type="hidden" name="hash" value="<?php
								echo $hash; ?>"/> <input type="hidden" name="txnid" value="<?php
								echo $txnid; ?>"/> <input type="hidden" name="phone" value="<?php
								echo $user['contact_no']; ?>"/> <input type="hidden" name="amount" value="<?php
								echo $pricepayu; ?>"/> <input type="hidden" name="firstname" id="firstname" value="<?php
								echo $user['first_name']; ?>"> <input type="hidden" name="email" id="email" value="<?php
								echo $user['email']; ?>"/> <input type="hidden" name="productinfo" value="<?php
								echo $this->lang->line('quiz_subscription'); ?>">
								<input type="hidden" name="surl" value="<?php
								echo site_url('payment_gateway_2/success/payu'); ?>" size="64"/>
								<input type="hidden" name="furl" value="<?php
								echo site_url('payment_gateway_2/cancel'); ?>" size="64"/>
								<input type="hidden" name="service_provider" value="payu_paisa" size="64"/>
								<input type="hidden" name="udf1" value="<?php
								echo $user['uid'] . '-' . str_replace(',', ':', $gids) . '-' . $quid; ?>">
								<input type="submit" value="<?php
								echo $this->lang->line('paynow'); ?>" class="btn btn-default">
							</form>
							<?php
						}
						?>
						...
						if ($this->config->item('paytm') == 'true') {
							...
							echo PAYTM_TXN_URL ?>" name="f1">
								<table border="1">
									...
									echo $checkSum ?>"> <input type="hidden" name="CALLBACK_URL" value="<?php
									echo site_url('payment_gateway_2/success/paytm'); ?>">
									</tbody>
								</table>
								<input type="submit" value="<?php
								echo $this->lang->line('paynow'); ?>" class="btn btn-default">
